"""Quotes_all Word template management service (runtime update)."""
...
from app.core.config import QUOTES_ALL_TEMPLATE_FALLBACK_PATH, QUOTES_ALL_TEMPLATE_PATH
...
def ensure_template_exists() -> Path:
    target = Path(QUOTES_ALL_TEMPLATE_PATH)
    if target.exists():
        return target

    fallback = Path(QUOTES_ALL_TEMPLATE_FALLBACK_PATH)
    if not fallback.exists():
        raise FileNotFoundError(
            f"Quotes_all template not found. Expected current={target} or fallback={fallback}"
        )
